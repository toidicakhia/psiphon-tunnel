...
TUNNEL_CORE_SRC_DIR=${GOPATH}/src/github.com/toidicakhia/psiphon-tunnel
...
ln -s "${BASE_DIR}/../.." "${GOPATH}/src/github.com/toidicakhia/psiphon-tunnel"
if [[ $? != 0 ]]; then
  echo "ln -s ../.. ${GOPATH}/src/github.com/toidicakhia/psiphon-tunnel"
  exit 1
fi
...
function gomobile_build_for_platform() {
  ...
  -tags="${BUILD_TAGS}" \
  -ldflags="${LDFLAGS}" \
  -o "${GOBIND_OUT}" github.com/toidicakhia/psiphon-tunnel/MobileLibrary/psi
...
cp -R "${GOPATH}/src/github.com/toidicakhia/psiphon-tunnel/MobileLibrary/go-mobile" "${GOPATH}/src/golang.org/x/mobile"
...
-s \
-w \
-X github.com/toidicakhia/psiphon-tunnel/psiphon/common/buildinfo.buildDate=${BUILDDATE} \
-X github.com/toidicakhia/psiphon-tunnel/psiphon/common/buildinfo.buildRepo=${BUILDREPO} \
-X github.com/toidicakhia/psiphon-tunnel/psiphon/common/buildinfo.buildRev=${BUILDREV} \
-X github.com/toidicakhia/psiphon-tunnel/psiphon/common/buildinfo.goVersion=${GOVERSION} \
"
